...
const firstLetters = (arr) => arr.map(first => first.charAt(0));
...
const findHappiness = (arr) => arr.filter(strings => strings.includes(':)'));
/* ------------------------------------------------------------------------------------------------
CHALLENGE 3
...
const standardizePhoneNumbers = (arr) => arr.map(phone => phone.replace(/[\(\)\- ]/g, ''));
...
const onlyOddChars = (str) => str.split('').filter((el, idx) => idx % 2 === 1).join('');
/* ------------------------------------------------------------------------------------------------
CHALLENGE 5
